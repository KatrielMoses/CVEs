...
Open source projects are the backbone of a huge amount of software that people trust without thinking twice about it. The developers building these projects are often doing it for free, in their own time, and security audits are expensive — most small to mid-sized projects will never get one.
...
|---------|-------|----------|--------|
| [jarrodwatts/claude-hud](https://github.com/jarrodwatts/claude-hud) — Claude Code statusline plugin | ~20k | 5 (2 High, 3 Medium) — shell injection in --extra-cmd, path traversal, LFI, OSC8 injection | CVEs pending |
| [tickstep/aliyunpan](https://github.com/tickstep/aliyunpan) — Alibaba Cloud Drive CLI client | ~5k | 8 (2 Critical, 4 High, 2 Medium) — RCE via updater ZipSlip, token relay to 3rd-party server, OS command injection | CVEs pending |
| [nsf/gocode](https://github.com/nsf/gocode) — Go autocompletion daemon for Vim/Neovim/Emacs | ~5k | 5 (2 Critical, 3 High) — unauthenticated RPC, RCE via GOPATH injection + autobuild chain | CVEs pending |
| [achristmascarl/rainfrog](https://github.com/achristmascarl/rainfrog) — Terminal database management tool | ~5k | 5 (5 High) — all exploitable without authentication | CVEs pending |
| [aaPanel/BaoTa](https://github.com/aaPanel/BaoTa) — BaoTa Linux server panel | ~4.4k | 2 (2 High) — predictable Flask SECRET_KEY, unauthenticated mail endpoint | CVEs pending |
| [xubiaolin/docker-zerotier-planet](https://github.com/xubiaolin/docker-zerotier-planet) — Self-hosted ZeroTier planet server | ~3.9k | 5 (1 Critical, 4 High) — all exploitable without prior knowledge of the deployment | CVEs pending |
| [Fredolx/open-tv](https://github.com/Fredolx/open-tv) — Cross-platform IPTV app | ~3.5k | 6 (1 Critical, 1 High, 4 Medium) — all reachable via malicious M3U or .otv share file | CVEs pending |
| [urwid/urwid](https://github.com/urwid/urwid) — Python console UI library | ~3k | 3 (2 High, 1 Medium) — web display backend exploitable on shared hosts, chained CVSS 9.0 | CVEs pending |
| [xyproto/algernon](https://github.com/xyproto/algernon) — Pure-Go web server with Lua scripting | ~3k | 4 (1 Critical, 3 High) — RCE via command injection, SSRF, path traversal, race condition | ✅ Patched in v1.17.6 |
| [Cp0204/quark-auto-save](https://github.com/Cp0204/quark-auto-save) — Quark cloud drive auto-save tool | ~2.3k | 2 (2 High) — mass assignment credential takeover, stored XSS | CVEs pending |
| [JuliusBrussee/cavekit](https://github.com/JuliusBrussee/cavekit) — Claude Code build orchestration plugin | growing | 5 (3 Critical, 2 High) — RCE via filename injection, prompt injection, WebSocket bypass, path traversal | CVEs pending |
